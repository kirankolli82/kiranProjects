...
public class Directory extends AbstractDiskResident {
    ...
    public Directory(String fileName, String absolutePath, long size,
                     FileTime createdTime, FileTime modifiedTime,
                     List<Directory> childDirectories, List<File> files) {
    ...
    public Iterator<DiskResident> getChildren() {
        return new Iterator<DiskResident>() {
            int index = 0;
            Iterator<Directory> directoryIterator = childDirectories.iterator();
    ...
    public DiskResidentType getType() {
        return DiskResidentType.DIRECTORY;
    }
}
